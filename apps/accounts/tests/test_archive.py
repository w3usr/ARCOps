...
def _user(email, level=AccessLevel.MEMBER, **kw):
    ...
    return User.objects.create_user(email, PASSWORD, access_level=level, **kw)
...
def test_archiving_keeps_everything_and_ends_access():
    ...
    assert member.access_level == AccessLevel.NONE  # and so sign-in is refused
...
def test_an_advisor_brings_a_former_member_back():
    ...
    member.refresh_from_db()
    assert not member.is_archived and member.access_level == AccessLevel.MEMBER
