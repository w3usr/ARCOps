...
FORM = {
    "callsign": "", "first_name": "New", "middle_name": "", "last_name": "Person", "preferred_name": "",
...
@pytest.fixture
def world():
    ClubSetting.objects.update_or_create(key="trusted_email_domains", defaults={"value": ["uni.example"]})
    ...
    ClubSetting.objects.update_or_create(
        ...
        defaults={"value": [{"key": "student", "label": "Student", "email_domain": "uni.example"}, {"key": "faculty", "label": "Faculty"}, {"key": "community", "label": "Community Member"}]},
    )
    ClubSetting.objects.update_or_create(
        key="slot_roles",
        defaults={"value": [{"key": "operator", "label": "Operator", "on_air": True}, {"key": "mentor", "label": "Mentor", "on_air": True, "requires_license": True}]},
    )
    off = User.objects.create_user("off@uni.example", "pw-Testing-123", access_level=AccessLevel.OFFICER, first_name="Ann", last_name="Officer")
    ev = Event.objects.create(title="RTTY", state=Event.State.PUBLISHED, created_by=off, min_license_class="General")
...
def _link(off, kind, **kw):
    return entry.create_link(off, label=kw.pop("label", "TEST 101"), kind=kind, expires_at=timezone.now() + timedelta(days=30), **kw)


def test_officer_creates_links_and_the_page_lists_them(world):
    c = _as(world["off"])
    r = c.post("/me/entry-links/", {"label": "PHYS 101 Fall", "kind": "class", "required_domain": "uni.example", "expires_at": "2030-01-01", "cap": "5", "landing_event": world["ev"].pk})
    link = EntryLink.objects.get(label="PHYS 101 Fall")
    body = r.content.decode()
    assert f"/join/{link.token}/" in body and link.required_domain == "uni.example" and link.cap == 5
    r = c.post("/me/entry-links/", {"label": "No domain", "kind": "class", "required_domain": "", "expires_at": "2030-01-01"})
    assert "needs a trusted domain" in r.content.decode()
    assert _as(User.objects.create_user("m@x.example", "pw-Testing-123", access_level=AccessLevel.MEMBER)).get("/me/entry-links/").status_code == 404
...
def test_class_link_admits_at_once_with_a_verification_deadline(world):
    ...
    c = Client()
    r = c.post(f"/join/{link.token}/form/", {**FORM, "email": "kid@home.example", "category": "student"})
    assert "is for uni.example addresses" in r.content.decode() and not User.objects.filter(email="kid@home.example").exists()
    r = c.post(f"/join/{link.token}/form/", {**FORM, "email": "kid@uni.example", "category": "student"})
    assert r.status_code == 302 and r["Location"] == f"/events/{world['ev'].pk}/"
    u = User.objects.get(email="kid@uni.example")
    assert u.access_level == "member" and u.joined_via == link and u.institution_email == "kid@uni.example"
...
def test_overdue_verification_pauses_sign_in_until_an_officer_waives_it(world):
    ...
    c = Client()
    c.post(f"/join/{link.token}/form/", {**FORM, "email": "late@uni.example", "category": "student"})
    ...
    c.post(f"/join/{link.token}/form/", {**FORM, "email": "late2@uni.example", "category": "student"})
...
def test_community_link_makes_a_provisional_member_after_verification_and_officers_hear(world):
    ...
    assert Outbox.objects.filter(user=world["off"], subject__startswith="New provisional member").exists()
...
def test_provisional_sees_counts_not_names_and_no_directory_or_agreements(world):
    link = _link(world["off"], "community", label="FRC 2026")
    prov = User.objects.create_user("p@gmail.example", "pw-Testing-123", access_level=AccessLevel.PROVISIONAL, first_name="Pat", last_name="Prov", joined_via=link)
    mem = User.objects.create_user("s@uni.example", "pw-Testing-123", access_level=AccessLevel.MEMBER, first_name="Stu", last_name="Dent", callsign="N0STU")
    ...
    assert c.get("/members/").status_code == 404 and c.get("/credentials/agreements/").status_code == 404
...
def test_decline_sets_no_access_with_a_reason(world):
    prov = User.objects.create_user("q@gmail.example", "pw-Testing-123", access_level=AccessLevel.PROVISIONAL, first_name="Q", last_name="R")
    _as(world["off"]).post(f"/members/{prov.pk}/", {"action": "decline", "reason": "not this season"})
    prov.refresh_from_db()
...
def test_existing_address_gets_the_same_page_and_an_email(world):
    link = _link(world["off"], "class", required_domain="uni.example")
    r = Client().post(f"/join/{link.token}/form/", {**FORM, "email": "off@uni.example", "category": "student"})
    assert "Check your email" in r.content.decode()
    assert Outbox.objects.filter(user=world["off"], body_html__contains="already has an account").exists()
