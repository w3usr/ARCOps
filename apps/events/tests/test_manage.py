...
def officer():
    ClubSetting.objects.update_or_create(
        key="slot_roles",
        defaults={"value": [{"key": "operator", "label": "Operator", "on_air": True}, {"key": "mentor", "label": "Mentor", "on_air": True}]},
    )
    return User.objects.create_user("off@example.org", "pw-Testing-123", access_level=AccessLevel.OFFICER, first_name="Ann", last_name="O")
...
def _build(c):
    c.post("/events/new/", {"title": "Field Day", "type": "contest", "description_html": "", "rules_url": "",
                            "min_license_class": "", "kbyg_html": "", "start": "2026-06-27T18:00", "end": "2026-06-28T18:00"})
    ev = Event.objects.get(title="Field Day")
    c.post(f"/events/{ev.pk}/locations/add/", {"name": "Club station", "position": "Run", "is_club_station": "on"})
    loc = ev.locations.get()
    c.post(f"/events/{ev.pk}/locations/{loc.pk}/positions/add/", {"name": "Mult"})
    c.post(f"/events/{ev.pk}/slots/generate/", {"minutes": "60", "setup_slots": "1", "breakdown_slots": "1", "cap_operator": "2", "cap_mentor": "1"})
    return ev
...
def test_members_are_kept_out_and_captains_let_in(officer):
    c = _as(officer)
    ev = _build(c)
    mem = User.objects.create_user("m@example.org", "pw-Testing-123", access_level=AccessLevel.MEMBER, first_name="Mo", last_name="M")
    cm = _as(mem)
...
def test_slot_close_cancel_duplicate_and_cancel_event(officer):
    ...
    assert Slot.objects.filter(position__location__event=copy).count() == 2 * 26 - 1  # the cancelled one is not copied
...
def test_grid_is_fixed_once_someone_has_signed_up(officer):
    ...
    c.post(f"/events/{ev.pk}/slots/generate/", {"minutes": "30", "setup_slots": "0", "breakdown_slots": "0", "cap_operator": "1", "cap_mentor": "0"})
