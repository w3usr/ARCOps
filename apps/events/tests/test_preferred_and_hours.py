...
def test_a_technician_makes_a_general_event_viable_with_a_warning():
    ...
    assert cell.shown.word == "Covered" and "below preferred class (Technician)" in cell.shown.detail and cell.shown.tone == "warn"
...
def test_mentor_needs_a_license_operator_does_not():
    ClubSetting.objects.filter(key="slot_roles").update(
        value=[{"key": "operator", "on_air": True}, {"key": "mentor", "on_air": True, "requires_license": True}, {"key": "observer", "on_air": False}]
    )
    s = slot_for("General")
    ...
    assert can_sign_up(person("tech", "Technician"), s, "mentor") == (True, "")  # any class; the event's class only warns
...
def test_names_carry_the_license_letter():
    ...
    assert person("gen", "General").license_letter == "G" and person("tech", "Technician").license_letter == "T"


def test_hours_credit_check_in_unless_no_show_and_export_csv():
    off = User.objects.create_user("off@example.org", "x", access_level=AccessLevel.OFFICER, first_name="Ann", last_name="O")
    link = EntryLink.objects.create(label="PHYS 101", kind="class", required_domain="example.org", expires_at=timezone.now() + dt.timedelta(days=9), created_by=off)
    stu = person("stu")
    stu.joined_via = link
    stu.save()
    s = slot_for()
