...
| `retention:apply` | `manage.py retention_apply` | daily | the retention schedule of REQUIREMENTS §4.3 (TR-28): responsible-adult records, message bodies, lapsed invitations, and the relationship note on an ended guardian link. It never touches a member's own record, which is archived and kept (FR-125). Skips accounts under legal hold; periods are `defaults.retention_*` |
