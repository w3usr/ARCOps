...
```
   ...
   |  environment file, root-owned, outside the checkout                  |
   |    (SECRET_KEY, field key, VAPID keys)                               |
   -----------------------------------------------------------------------
                 |                                    ^
...
- **TR-2 Datastore: SQLite in WAL mode**, one file under the deployment's own writable
  directory, accessed only by the application. *Why:* the club has tens of members and a few hundred writes on a
...
- **TR-18 Rate limiting: nginx `limit_req` on the sign-in, reset, and invitation endpoints**,
  ...
  per-account rule.
- **TR-19 Secrets: one root-owned environment file on the server**, mode 0600, outside the
  checkout, its location a matter for whoever runs the installation. It holds `SECRET_KEY`, the
  field-encryption key (TR-20), and the VAPID key pair; the deployment generates it on first run,
  it is committed to no repository, it is recorded in that deployment's own credential inventory,
  and it is rotated on turnover per that inventory's rules. The application refuses to start without it. *Why:* the
...
- **TR-22 Backups: nightly, encrypted, pulled off the box at no cost** (adopted 2026-09-13; the
  ...
  in the deployment's backup directory with seven days retained. A cron job on a campus machine the
...
- **TR-41 Club overlay: W3USR's assets are tracked privately and laid over the defaults at
  deploy.** The application reads an **overlay directory** (`CLUB_OVERLAY_DIR`, wherever the deployment
  puts it) with the same layout as `config/` (`club.yaml`, `static/club/…`,
